...
<customTag1>X6</customTag> = (<customTag1>X1</customTag> + <customTag1>X4</customTag>) /* IEXPRBIN */;//event.Local

<customTag1>X5</customTag> = atomic_load_explicit(&<customTag1>X6</customTag>/*com.dat3m.dartagnan.program.Register*/, memory_order_acquire);//event.Load
...
assert((((x == 3) && (<customAssertionTag0>X3</customAssertionTag> == 3)) && (<customAssertionTag1>X5</customAssertionTag> == 0)));
return 0;
}
